...
export const Navbar: React.FC = () => {
  ...
  const { register, handleSubmit, watch, reset, formState: { isSubmitting } } = useForm<SearchFormValues>({
  ...
  const onSubmit = async (data: SearchFormValues) => {
    ...
    setResults(null);
    try {
      const recommendations = await searchProductsWithGemini(data.query);
              ...
              <div className="relative flex items-center bg-white rounded-md overflow-hidden h-11 md:h-12 transition-all duration-300 focus-within:ring-2 focus-within:ring-red-500/50">
                ...
                {queryValue && (
                  <button 
                    type="button"
                    onClick={() => reset()}
                    className="p-2 text-gray-400 md:hover:text-gray-600 active:text-gray-600"
                  >
                    <X size={16} />
                  </button>
                )}
